...
/datum/antagPopups/proc/generateHTML()
	if (html)
		html = ""
...
/datum/antagPopups/proc/showPanel()
	if (!html)
		if (!generateHTML())
...
/datum/antagPopups/proc/searchWiki(mob/M, phrase)
	M << link("https://wiki.ss13.co/index.php?search=[phrase]")

/datum/antagPopups/Topic(href, href_list)
	if (href_list["action"])
...
/datum/antagPopups/proc/show_popup(mob/M, popup_name)
...
/mob/var/last_antag_popup = null

/**
 * Show antag popup with to a mob.
 *
 * Arguments:
 * - popup_name: The name of the popup to match to the correct macro.
 * - set_last_popup: Whether to modify the mob's last_antag_popup entry (used for the admin display).
 */
/mob/proc/show_antag_popup(popup_name, set_last_popup = TRUE)
	if (set_last_popup)
		src.last_antag_popup = popup_name
	get_singleton(/datum/antagPopups).show_popup(src, popup_name)

/mob/verb/reopen_antag_popup()
