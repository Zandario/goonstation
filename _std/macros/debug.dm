...
/proc/stack_trace(thing_to_crash)
...
/datum/dm_dumper/proc/make_var(name)
	name = ckey(name)
	. = name
	var/i = 2
	while(. in tmp_vars)
...
/datum/dm_dumper/proc/image_constructor(image/img)
	var/list/cargs = list()
	var/i = 0
	var/n_args = 0
...
/datum/dm_dumper/proc/dm_dump(x, list/lines, list/path, return_main=FALSE, new_params=null, no_path_on_main=FALSE, can_use_ref=TRUE)
	if(--steps_limit <= 0)
		return
	var/val = null
	if(!return_main)
