 ...
 */
/atom/movable/screen/plane_parent/New(plane, appearance_flags = 0, blend_mode = BLEND_DEFAULT, color, mouse_opacity = 1, name = "unnamed_plane", is_screen = 0)
	src.name = name
	src.plane = plane
...
/atom/movable/screen/plane_parent/proc/add_depth_shadow()
...
/atom/movable/screen/plane_display/New(atom/movable/screen/plane_parent/pl)
	if(pl)
		src.name = pl.name
...
/atom/movable/screen/plane_display/master/proc/request_keep_together()
	src.keep_together_requests++
	src.appearance_flags |= KEEP_TOGETHER

/atom/movable/screen/plane_display/master/proc/release_keep_together()
	src.keep_together_requests--
...
/client/proc/add_plane(atom/movable/screen/plane_parent/plane)
	RETURN_TYPE(/atom/movable/screen/plane_parent)
	src.plane_parents["[plane.plane]"] = plane
	return plane

/client/proc/apply_depth_filter()
...
/client/proc/setup_special_screens()
	for(var/plane_key in src.plane_parents)
		var/atom/movable/screen/plane_parent/P = src.plane_parents[plane_key]
		screen += P

/client/proc/get_plane(plane)
	RETURN_TYPE(/atom/movable/screen/plane_parent)
