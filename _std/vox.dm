...
/datum/VOXsound/New(id, file, flags)
	..()
	src.id = id
	src.ogg = file
	src.flags = flags

/datum/VOXsound/disposing()
	ogg = null
	soundFile = null
	voxsounds -= src
...
/datum/VOXsound/proc/matches_id(t)
	. = id == lowertext(t)

/datum/VOXsound/proc/matches_flag(f)
	. = 0
	if (istext(f))
		f = text2num(f)
	if (isnum(f))
...
/datum/VOXsound/proc/load(freq = 1)
	if (src.ogg)
...
/datum/VOXsound/proc/play(client/listener)
	if (src.soundFile)
		listener << src.soundFile

/datum/voxdbg
	var/list/voxtokens = list()
	var/list/voxsounds_flag_sorted = list()
	var/list/voxsounds = list()
...
/proc/vox_play(input, user, pitch = 1)
	if(!input)
		return
...
/proc/vox_playerfilter(input as text)
	if(!vox_banned_words)
