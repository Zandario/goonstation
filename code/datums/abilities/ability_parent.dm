...
/datum/abilityHolder/proc/onLife(mult = 1) //failsafe for UI not doing its update correctly elsewhere
	.= 0
	if (points_last != points)
		points_last = points
		src.updateText(0, src.x_occupied, src.y_occupied)

/datum/abilityHolder/proc/updateCounters()
...
/datum/abilityHolder/proc/updateButtons(called_by_owner = 0, start_x = 1, start_y = 0)
...
/datum/abilityHolder/proc/updateText(called_by_owner = 0)
	if (composite_owner && !called_by_owner)
...
/datum/abilityHolder/proc/deepCopy()
...
/datum/abilityHolder/proc/addBonus(value)
	bonus += value

/datum/abilityHolder/proc/generatePoints(mult = 1)
	lastBonus = bonus
	points += bonus
	points += regenRate * mult
	bonus = 0

/datum/abilityHolder/proc/transferOwnership(newbody)
	owner?.detach_hud(hud)
	owner = newbody
	if(owner)
		owner.attach_hud(hud)

/datum/abilityHolder/proc/Stat()
...
/datum/abilityHolder/proc/StatAbilities()
	if (!rendered)
		return
...
/datum/abilityHolder/proc/onAbilityStat()
	return

/datum/abilityHolder/proc/deductPoints(cost, target_ah_type)
	if (!usesPoints || cost == 0)
		return

	points -= cost

/datum/abilityHolder/proc/addPoints(add_points, target_ah_type)
	if (!usesPoints)
		return

	points += add_points

/datum/abilityHolder/proc/suspendAllAbilities()
	src.suspended = src.abilities.Copy()
	src.abilities.len = 0
	src.updateButtons()

/datum/abilityHolder/proc/resumeAllAbilities()
	if (src.suspended && length(src.suspended))
		src.abilities = src.suspended
		src.suspended = list()
	src.updateButtons()

/datum/abilityHolder/proc/addAbility(abilityType)
...
/datum/abilityHolder/proc/removeAbility(abilityType)
	if (istext(abilityType))
		abilityType = text2path(abilityType)
	if (!ispath(abilityType))
		return
...
/datum/abilityHolder/proc/removeAbilityInstance(datum/targetable/A)
	if (!istype(A))
		return
...
/datum/abilityHolder/proc/getAbility(abilityType)
	if (!ispath(abilityType))
		return null
	for (var/datum/targetable/A in src.abilities)
		if (A.type == abilityType)
...
/datum/abilityHolder/proc/pointCheck(cost)
	if (!usesPoints)
		return 1
...
/datum/abilityHolder/proc/click(atom/target, params)
	if (!owner)
		return 0
	if (params["alt"])
		if (altPower)
...
/datum/abilityHolder/proc/actionKey(num)
...
/datum/abilityHolder/proc/cancel_action_binding()
	for (var/datum/targetable/T in src.abilities)
		T.waiting_for_hotkey = 0
	updateButtons()

/datum/abilityHolder/proc/unbind_action_number(num)
	for (var/datum/targetable/T in src.abilities)
		if(T.action_key_number == num)
...
/datum/abilityHolder/proc/onAbilityHolderInstanceAdd()
	return 0

/datum/abilityHolder/proc/remove_unlocks()
	return 0

/datum/abilityHolder/proc/set_loc_callback(newloc)
	.=0
...
/datum/abilityHolder/proc/get_controlling_mob()
...
/atom/movable/screen/ability/disposing()
	src.screen_loc = null
	owner = null
	..()
...
/atom/movable/screen/ability/proc/set_number_overlay(num)
 ...
 */
/atom/movable/screen/ability/proc/do_target_selection_check()
...
//WIRE TOOLTIPS
/atom/movable/screen/ability/MouseEntered(location, control, params)
...
/atom/movable/screen/ability/MouseExited()
...
/atom/movable/screen/abilitystat/New()
	..()
...
/atom/movable/screen/abilitystat/disposing()
	if(owner?.hud)
		owner.hud.remove_object(src)
	..()

/atom/movable/screen/abilitystat/proc/get_controlling_mob()
	var/mob/M = owner.get_controlling_mob()
	if (!istype(M) || !M.client)
		return null
	return M

/atom/movable/screen/abilitystat/proc/update_on_hud(pos_x = 0, pos_y = 0)
...
/atom/movable/screen/ability/topBar/New()
	..()
...
/atom/movable/screen/ability/topBar/disposing()
	qdel(point_overlay)
	point_overlay = null
...
/atom/movable/screen/ability/topBar/update_icon()
...
/atom/movable/screen/ability/topBar/proc/get_controlling_mob()
...
/atom/movable/screen/ability/topBar/proc/update_cooldown_cost()
...
/atom/movable/screen/ability/topBar/proc/update_on_hud(pos_x = 0, pos_y = 0)
...
/atom/movable/screen/ability/topBar/clicked(parameters)
...
/atom/movable/screen/ability/topBar/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
...
/datum/targetable
	...
	var/target_in_inventory = 0
	var/last_cast = 0
	var/cooldown = 100
	var/start_on_cooldown = 0
	...
	var/sticky = 0
	/// If 1, Ability will stick to cursor even if ability goes on cooldown after first cast.
	var/ignore_sticky_cooldown = 0
	/// If 1, we will interrupt any action bars running with the INTERRUPT_ACT flag
	var/interrupt_action_bars = 1

	/// Number hotkey assigned to this ability. Only used if > 0
	var/action_key_number = -1
	/// If 1, the next number hotkey pressed will be bound to this.
	var/waiting_for_hotkey = 0

	var/preferred_holder_type = /datum/abilityHolder

	var/icon = 'icons/mob/spell_buttons.dmi'
	var/icon_state = "blob-template"

	/// For wire's tooltips, it's about time this got varized
	var/theme = null
	var/tooltip_flags = null

/// DON'T OVERRIDE THIS. OVERRIDE onAttach()!
/datum/targetable/New(datum/abilityHolder/holder)
...
/datum/targetable/proc/handleCast(atom/target, params)
...
/datum/targetable/proc/cast(atom/target)
...
/datum/targetable/proc/onAttach(datum/abilityHolder/H)
 ...
 */
/datum/targetable/proc/tryCast(atom/target, params)
...
/datum/targetable/proc/updateObject()
	return

/datum/targetable/proc/doCooldown()
	src.last_cast = world.time + src.cooldown

/datum/targetable/proc/castcheck(atom/target)
	return TRUE

/datum/targetable/proc/cooldowncheck()
	if (src.last_cast > world.time)
		return FALSE
	return TRUE

/datum/targetable/proc/afterCast()
	return

/datum/targetable/proc/Stat()
	updateObject(holder.owner)
	stat(null, object)

/// Universal grab check you can use (Convair880).
/datum/targetable/proc/grab_check(mob/target, state = 1, dirty = 0)
	if (!holder || state < 1)
		return 0
...
///area See comment in /atom/movable/screen/ability (Convair880).
/datum/targetable/proc/target_reference_lookup()
...
/datum/targetable/proc/display_available()
	.= (src.icon && src.icon_state)

/datum/targetable/proc/flip_callback()
	.= 0
...
/datum/abilityHolder/composite/disposing()
...
/datum/abilityHolder/composite/proc/addHolder(holderType)
	for (var/datum/abilityHolder/H in holders)
...
/// Return holder on success, null on fail
/datum/abilityHolder/composite/proc/addHolderInstance(var/datum/abilityHolder/N, keep_owner = FALSE)
	for (var/datum/abilityHolder/H in holders)
		if (H == N)
			return
	holders += N
...
/datum/abilityHolder/composite/proc/removeHolder(holderType)
	for (var/datum/abilityHolder/H in holders)
		if (H.type == holderType)
...
/datum/abilityHolder/composite/proc/getHolder(holderType)
	for (var/datum/abilityHolder/H in holders)
		if (H.type == holderType)
			return H

/datum/abilityHolder/composite/cancel_action_binding()
	for (var/datum/abilityHolder/H in holders)
		H.cancel_action_binding()

/datum/abilityHolder/composite/unbind_action_number(num)
	for (var/datum/abilityHolder/H in holders)
		H.unbind_action_number(num)
	return 0

/datum/abilityHolder/composite/actionKey(num)
	var/used = 0
...
/datum/abilityHolder/composite/updateCounters()
	var/num = 0
	for (var/datum/abilityHolder/H in holders)
		H.updateCounters()
		num++
...
/datum/abilityHolder/composite/updateButtons(called_by_owner = 0, start_x = 1, start_y = 0)
...
/datum/abilityHolder/composite/updateText(called_by_owner = 0)
	if (!abilitystat)
		abilitystat = new
		abilitystat.owner = src
...
/datum/abilityHolder/composite/click(atom/target, params)
...
/datum/abilityHolder/composite/addBonus(value)
	for (var/datum/abilityHolder/H in holders)
		H.addBonus(value)

/datum/abilityHolder/composite/generatePoints(mult = 1)
	for (var/datum/abilityHolder/H in holders)
		H.generatePoints(mult)

/datum/abilityHolder/composite/Stat()
	for (var/datum/abilityHolder/H in holders)
		H.Stat()

/datum/abilityHolder/composite/StatAbilities()
	for (var/datum/abilityHolder/H in holders)
		H.StatAbilities()

/datum/abilityHolder/composite/deductPoints(cost, target_ah_type)
	for (var/datum/abilityHolder/H in holders)
		if (target_ah_type && !istype(H, target_ah_type))
			continue
		H.deductPoints(cost)

/datum/abilityHolder/composite/addPoints(add_points, target_ah_type)
	for (var/datum/abilityHolder/H in holders)
		if (target_ah_type && !istype(H, target_ah_type))
			continue
		H.addPoints(add_points)

/datum/abilityHolder/composite/suspendAllAbilities()
	for (var/datum/abilityHolder/H in holders)
		H.suspendAllAbilities()

/datum/abilityHolder/composite/resumeAllAbilities()
	for (var/datum/abilityHolder/H in holders)
		H.resumeAllAbilities()

/datum/abilityHolder/composite/addAbility(abilityType)
	//why was this? Weird
	// if (!holders.len)
	// 	return
	if (istext(abilityType))
...
/datum/abilityHolder/composite/removeAbility(abilityType)
	if (istext(abilityType))
		abilityType = text2path(abilityType)
	if (!ispath(abilityType))
		return
...
/datum/abilityHolder/composite/removeAbilityInstance(datum/targetable/A)
	if (!istype(A))
		return
	for (var/datum/abilityHolder/H in holders)
...
/datum/abilityHolder/composite/getAbility(abilityType)
	if (!ispath(abilityType))
		return null
	for (var/datum/abilityHolder/H in holders)
...
/datum/abilityHolder/composite/pointCheck(cost)
	return 1

/datum/abilityHolder/composite/deepCopy()
	var/datum/abilityHolder/composite/copy = new src.type(src.owner)
	for (var/datum/abilityHolder/H in holders)
		copy.holders += H.deepCopy()
	return copy

/datum/abilityHolder/composite/transferOwnership(newbody)
	for (var/datum/abilityHolder/H in holders)
		H.transferOwnership(newbody)
	..()

/datum/abilityHolder/composite/remove_unlocks()
	for (var/datum/abilityHolder/H in holders)
		H.remove_unlocks()

/datum/abilityHolder/composite/onLife(mult = 1)
	for (var/datum/abilityHolder/H in holders)
		H.onLife(mult)

/datum/abilityHolder/composite/set_loc_callback(newloc)
	for (var/datum/abilityHolder/H in holders)
