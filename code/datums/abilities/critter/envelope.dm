...
/datum/action/bar/icon/envelopAbility/critter
	duration = 6 SECONDS

/datum/action/bar/icon/envelopAbility/New(Target, Envelop)
	target = Target
	ability = Envelop
	..()

/datum/action/bar/icon/envelopAbility/onUpdate()
	..()
...
/datum/action/bar/icon/envelopAbility/onStart()
	..()
	if (BOUNDS_DIST(owner, target) > 0 || target == null || owner == null || (ability && !ability.cooldowncheck()))
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/envelopAbility/onEnd()
	..()
...
/datum/targetable/critter/envelop/proc/actionFinishCooldown()
	cooldown = actual_cooldown
	doCooldown()
	cooldown = initial(cooldown)

/datum/targetable/critter/envelop/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
