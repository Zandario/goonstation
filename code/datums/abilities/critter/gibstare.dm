...
/datum/action/bar/icon/gibstareAbility/New(Target, Gibstare, new_duration, new_max_range = 10)
	target = Target
	ability = Gibstare
	if(new_duration)
...
/datum/action/bar/icon/gibstareAbility/onUpdate()
...
/datum/action/bar/icon/gibstareAbility/onStart()
	..()
...
/datum/targetable/critter/gibstare/proc/actionFinishCooldown()
	cooldown = actual_cooldown
	doCooldown()
	cooldown = initial(cooldown)

/datum/targetable/critter/gibstare/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
