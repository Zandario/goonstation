...
/datum/targetable/critter/psyblast/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
	...
	if (!istype(MT))
		boutput(holder.owner, "<span class='alert'>Nothing to psyblast there.</span>")
		return TRUE
	playsound(MT.loc, 'sound/effects/ghost2.ogg', 100, TRUE)
...
/datum/targetable/critter/psyblast/martian/cast(atom/target)
	if (..())
		return TRUE
