...
/datum/targetable/critter/takepicture/cast(atom/target)
	if (..())
		return TRUE
	playsound(target, "sound/items/polaroid[rand(1,2)].ogg", 100, TRUE, -1)
	if (!target)
		return FALSE
...
/datum/targetable/critter/flash/cast(atom/target)
	if (..())
		return TRUE
	...
	playsound(holder.owner, 'sound/weapons/flash.ogg', 100, TRUE)
...
/datum/targetable/critter/control_owner/cast(atom/target)
	if (..())
		return TRUE
...
/datum/targetable/critter/control_owner/incapacitationCheck()
	return FALSE
...
/datum/targetable/critter/scuttle_scan/cast(atom/target)
	if (..())
		return TRUE
