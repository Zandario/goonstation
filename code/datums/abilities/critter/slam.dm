...
/datum/projectile/slam/on_launch(obj/projectile/O)
...
/datum/projectile/slam/tick(obj/projectile/O)
	if (O.disposed)
		return
...
/datum/projectile/slam/on_hit(atom/hit, angle, obj/projectile/O)
...
/datum/targetable/critter/slam/cast(atom/target)
	if (..())
		return TRUE
...
/datum/targetable/critter/slam_polymorph/cast(atom/target)
	if (..())
		return TRUE
	var/turf/T = get_turf(target)
	if (!T)
