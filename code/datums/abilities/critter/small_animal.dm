...
/datum/targetable/critter/bite/small/cast(atom/target)
	if (..())
		return TRUE
	playsound(target, src.sound_bite, 100, TRUE, -1)
	var/mob/MT = target
...
/datum/targetable/critter/peck/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
	...
	if (iscarbon(MT) && prob(60))
		...
		playsound(target, "swing_hit", 30, FALSE)
...
/datum/targetable/critter/pounce/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
		target = get_turf(target)
	...
	if (prob(25))
		holder.owner.visible_message(
			"<span class='combat'><B>[holder.owner]</B> weaves around [MT]'s legs and trips [him_or_her(MT)]!</span>",
			"<span class='combat'>You weave around [MT]'s legs and trip [him_or_her(MT)]!</span>")
		MT.changeStatus("weakened", 2 SECONDS)
		return FALSE
	else
		holder.owner.visible_message(
			"<span class='combat'><B>[holder.owner]</B> pounces on [MT]!</span>",
...
/datum/targetable/critter/trip/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
		target = get_turf(target)
...
/datum/targetable/critter/wasp_sting/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
		target = get_turf(target)
...
/datum/targetable/critter/wasp_sting/scorpion_sting
...
/datum/targetable/critter/pincer_grab/cast(atom/target)
	if (!holder)
		return
	...
	var/mob/MT = target
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] grabs [MT] with [his_or_her(holder.owner)] pincers!</b></span>",
...
/datum/targetable/critter/hootat/cast(atom/target)
	if (disabled)
		return TRUE
	if (..())
		return TRUE
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] hoots seductively!</b></span>",
		"<span class='combat'>You hoot seductively!</span>")
	playsound(holder.owner, 'sound/voice/animal/hoot.ogg', 90, FALSE)
