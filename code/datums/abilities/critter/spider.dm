...
/datum/targetable/critter/spider_bite/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
	...
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] bites [MT]!</b></span>",
	...
	if (istype(S))
		...
		playsound(holder.owner, 'sound/weapons/handcuffs.ogg', 50, TRUE, pitch = 1.6)
		if (issilicon(MT))
...
/datum/targetable/critter/spider_flail/cast(atom/target)
	...
	var/mob/living/critter/spider/S = holder.owner
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] dives on [MT]!</b></span>",
		"<span class='combat'><b>You dive on [MT]!</b></span>")
	playsound(holder.owner, 'sound/impact_sounds/Generic_Shove_1.ogg', 50, FALSE, pitch = 1.6)
	logTheThing(LOG_COMBAT, S, "used their [src.name] ability on [MT] at [log_loc(S)]")
	...
	SPAWN(0)
		...
		while (flail > 0 && MT && !MT.disposed)
			...
			if (istype(S))
				...
				playsound(holder.owner, 'sound/weapons/handcuffs.ogg', 50, TRUE)
				if (issilicon(MT))
					var/mob/living/silicon/robot/R = MT
					R.compborg_take_critter_damage("[pick("l","r")]_[pick("arm","leg")]", rand(2,4))
				else
...
/datum/targetable/critter/spider_drain/cast(atom/target)
	if (disabled && world.time > last_cast)
		disabled = FALSE // break the deadlock
	if (disabled)
		return TRUE
	...
	var/mob/living/critter/spider/S = holder.owner
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] starts draining the fluids out of [H]!</b></span>",
		"<span class='combat'><b>You start draining the fluids out of [H]!</b></span>")
	playsound(holder.owner, 'sound/misc/pourdrink.ogg', 50, FALSE, pitch = 0.7)
	...
	SPAWN(0)
		...
		if (H && H.stat && holder.owner.loc == H.loc)
			holder.owner.visible_message(
				"<span class='combat'><b>[src] drains [H] dry!</b></span>",
				"<span class='combat'><b>You drain [H] dry!</b></span>")
			H.death(FALSE)
			H.real_name = "Unknown"
			if (H.bioHolder)
				H.bioHolder.AddEffect("husk")
			playsound(holder.owner, 'sound/misc/fuse.ogg', 50, TRUE)
			...
			SPAWN(0)
				...
				if (istype(S))
					switch (S.encase_in_web)
						if (2)
							holder.owner.visible_message(
								"<span class='combat'><b>[holder.owner] encases [H] in cotton candy!</b></span>",\
							...
							cube.steam_on_death = FALSE

						if (1)
							holder.owner.visible_message(
								"<span class='combat'><b>[holder.owner] encases [H] in web!</b></span>",\
						...
						if (0)
							holder.owner.visible_message(
								"<span class='combat'><b>[holder.owner] encases [H] in ice!</b></span>",\
...
/datum/targetable/critter/clownspider_kick/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
		target = get_turf(target)
	...
	playsound(holder.owner, "swing_hit", 30, FALSE)
	if (prob(10))
		playsound(holder.owner, src.sound_kick, 50, FALSE)
	return FALSE
...
/datum/targetable/critter/clownspider_trample/cast(atom/target)
	if (disabled && world.time > last_cast)
		disabled = FALSE // break the deadlock
	if (disabled)
		return TRUE
	...
	var/mob/MT = target
	holder.owner.visible_message(
		"<span class='combat'><b>[holder.owner] pounces on top of [MT]!</b></span>",
		"<span class='combat'><b>You pounce onto [MT]!</b></span>")
	playsound(holder.owner, 'sound/impact_sounds/Generic_Shove_1.ogg', 50, FALSE)
	MT.TakeDamageAccountArmor("All", rand(4,10), 0, 0, DAMAGE_STAB)
	if (!isdead(MT))
		MT.emote("scream")
	disabled = TRUE
	SPAWN(0)
		...
		while (flail > 0 && MT && !MT.disposed)
			...
			if (holder.owner.getStatusDuration("stunned") || holder.owner.getStatusDuration("weakened") || holder.owner.getStatusDuration("paralysis"))
				break
			playsound(holder.owner, 'sound/impact_sounds/Flesh_Break_1.ogg', 50, TRUE)
			playsound(holder.owner, src.sound_kick, 50, TRUE)
			if (issilicon(MT))
				var/mob/living/silicon/robot/R = MT
			...
			holder.owner.visible_message(
				"<span class='combat'><b>[holder.owner] stomps on [MT]!</b></span>",
...
/datum/targetable/critter/vomitegg/cast(atom/T)
