...
/datum/action/bar/icon/zombifyAbility/New(Target, Zombify)
	target = Target
	zombify = Zombify
	..()

/datum/action/bar/icon/zombifyAbility/onUpdate()
	..()
...
/datum/action/bar/icon/zombifyAbility/onStart()
	..()
	if(BOUNDS_DIST(owner, target) > 0 || target == null || owner == null || target == owner || !zombify || !zombify.cooldowncheck())
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/zombifyAbility/onEnd()
	..()
...
/datum/targetable/critter/zombify/proc/actionFinishCooldown()
	cooldown = actual_cooldown
	doCooldown()
	cooldown = initial(cooldown)

/datum/targetable/critter/zombify/cast(atom/target)
	if (..())
		return TRUE
	if (isobj(target))
