...
/atom/movable/screen/ability/topBar/kudzu/clicked(params)
...
/datum/abilityHolder/kudzu/disposing()
	qdel(nutrients_meter)
	qdel(growth_amt)
	..()

/datum/abilityHolder/kudzu/onLife(mult = 1)
	if(..())
		return
	if (nutrients_meter)
...
/datum/targetable/kudzu/onAttach(datum/abilityHolder/H)
	..()
...
/datum/targetable/kudzu/updateObject()
	..()
	if (!src.object)
...
/datum/targetable/kudzu/castcheck()
	if (!holder)
		return 0
...
/datum/targetable/kudzu/cast(atom/target)
	. = ..()
...
/datum/targetable/kudzu/guide/cast(atom/tar)
...
/obj/kudzu_marker/New(location as turf)
	..()
	icon_state = "kudzu-benign-[rand(1,3)]"
	var/turf/T = get_turf(location)
	T.temp_flags |= HAS_KUDZU

/obj/kudzu_marker/set_loc(newloc as turf|mob|obj in world)
...
/obj/kudzu_marker/disposing()
	var/turf/T = get_turf(src)
	T.temp_flags &= ~HAS_KUDZU
	..()

//mostly same as kudzu
/obj/kudzu_marker/attackby(obj/item/W, mob/user)
	if (!W)
		return
	if (!user)
		return
...
/datum/targetable/kudzu/stealth/cast(atom/T)
...
/datum/targetable/kudzu/heal_other/cast(atom/target)
	if (..())
		return 1
...
/datum/targetable/kudzu/kudzusay/cast(atom/target)
	if (..())
		return 1
...
/datum/targetable/kudzu/seed/cast(atom/T)
...
/datum/targetable/kudzu/seed/proc/manipulate(obj/item/seed/S, power as num, mob/user)
	if (!istype(S))
		return //Should never happen but byond has ruined me.
...
/datum/targetable/kudzu/seed/proc/create(power as num)
...
/datum/targetable/kudzu/growth/cast(atom/target)
...
/datum/targetable/kudzu/vine_appendage/New(datum/abilityHolder/kudzu/holder)
	..(holder)

	vine = new/obj/item/kudzu/kudzumen_vine(holder?.owner)		//make the vine item in

/datum/targetable/kudzu/vine_appendage/cast()
...
/datum/targetable/kudzu/vine_appendage/proc/attempt_vine_drop(obj/item/kudzu/kudzumen_vine/V, mob/owner)
...
/atom/movable/screen/kudzu/New(var/datum/abilityHolder/kudzu/holder)
	..()
	src.holder = holder
...
//WIRE TOOLTIPS
/atom/movable/screen/kudzu/meter/MouseEntered(location, control, params)
...
/atom/movable/screen/kudzu/meter/MouseExited()
	if (usr.client.tooltipHolder)
		usr.client.tooltipHolder.hideHover()

/atom/movable/screen/kudzu/meter/proc/update()		//getting weird numbers in here
...
/atom/movable/screen/kudzu/growth_amount/New(datum/abilityHolder/kudzu/holder, datum/controller/process/kudzu/K)
	..(holder)
	if (istype(K))
		kudzu_controller = K
...
/atom/movable/screen/kudzu/growth_amount/disposing()
	kudzu_controller = null
	..()

//WIRE TOOLTIPS
/atom/movable/screen/kudzu/growth_amount/MouseEntered(location, control, params)
	if (usr.client.tooltipHolder && control == "mapwindow.map")
		var/theme = src.theme
...
/atom/movable/screen/kudzu/growth_amount/MouseExited()
	if (usr.client.tooltipHolder)
		usr.client.tooltipHolder.hideHover()

/atom/movable/screen/kudzu/growth_amount/proc/update()
...
/obj/item/kudzu/kudzumen_vine/New()
	..()
	src.build_buttons()
	src.setItemSpecial(/datum/item_special/rangestab)

/obj/item/kudzu/kudzumen_vine/dropped(mob/user)
	..()
	if (iskudzuman(user))
		src.set_loc(user)
...
/obj/item/kudzu/kudzumen_vine/attack(mob/M, mob/user, def_zone, is_special = 0)
	..()
...
/datum/action/bar/icon/kudzu_shaping/New(obj/spacevine/kudzu, obj/item/kudzu/kudzumen_vine/vine_arm, creation_path, extra_time as num)
	src.kudzu = kudzu
...
/datum/action/bar/icon/kudzu_shaping/onUpdate()
	..()
...
/datum/action/bar/icon/kudzu_shaping/onStart()
	..()
	if(BOUNDS_DIST(owner, kudzu) > 0 ||  kudzu == null || kudzu.growth < 20 || owner == null)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/kudzu_shaping/onEnd()
	..()
	if(BOUNDS_DIST(owner, kudzu) > 0 || kudzu == null || kudzu.growth < 20 || owner == null)
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/kudzu_shaping/onInterrupt()
	if (kudzu && owner)
...
/proc/destroys_kudzu_object(obj/O, obj/item/W as obj, mob/user)
