...
/datum/abilityHolder/revenant/Stat()
	if (relay)
		relay.Stat()
	if (owner)
		stat("Human vessel integrity:", "[(owner.max_health + 50) / 1.5]%")

/datum/abilityHolder/revenant/generatePoints(mult = 1)
	if (relay)
		relay.generatePoints(mult)

/datum/abilityHolder/revenant/deductPoints(cost)
	if (relay)
		return relay.deductPoints(cost)
	return 1

/datum/abilityHolder/revenant/pointCheck(cost)
	if (!relay)
		return 1
	if (!relay.usesPoints)
		return 1
...
/datum/abilityHolder/revenant/onAbilityStat()
	..()
	.= list()
...
/datum/bioEffect/hidden/revenant/OnAdd()
...
/datum/bioEffect/hidden/revenant/OnRemove()
	if (owner)
...
/datum/bioEffect/hidden/revenant/proc/ghoulTouch(mob/living/carbon/human/poorSob, obj/item/affecting)
...
/datum/bioEffect/hidden/revenant/proc/wraithPossess(mob/wraith/W)
	if (!W.mind && !W.client)
		return
	...
	owner.visible_message(
		"<span class='alert'><strong>[pick("[owner] suddenly rises from the floor!", "[owner] suddenly looks a lot less dead!", "A dark light shines from [owner]'s eyes!")]</strong></span>",\
...
/datum/bioEffect/hidden/revenant/proc/RevenantDeath()
	if (isDying)
		return
	isDying = 1
	if (!src.wraith)
...
/datum/bioEffect/hidden/revenant/OnLife(mult)
	if (!src.wraith)
		return
	if (ghoulTouchActive)
...
/datum/bioEffect/hidden/revenant/proc/addRevenantVerbs()
...
/*
/datum/bioEffect/hidden/revenant/proc/removeRevenantVerbs()
	if (owner.mind)
		owner.mind.spells.len = 0
	return
*/

/atom/movable/screen/ability/topBar/revenant/update_cooldown_cost()
	var/newcolor = null
...
/datum/targetable/revenantAbility/New()
...
/datum/targetable/revenantAbility/cast(atom/target)
	return

/datum/targetable/revenantAbility/castcheck()
	if (holder?.owner)
		return 1
	else
...
/datum/targetable/revenantAbility/doCooldown()
	if (!holder)
		return
...
/datum/targetable/revenantAbility/massCommand/cast(atom/target)
...
/datum/targetable/revenantAbility/shockwave/proc/shock(turf/T)
...
/datum/targetable/revenantAbility/shockwave/cast()
	var/list/next = list()
	var/list/NN = list()
...
/datum/targetable/revenantAbility/touchOfEvil/cast()
...
/datum/targetable/revenantAbility/push/cast(atom/target)
...
/datum/targetable/revenantAbility/crush/cast(atom/target)
...
/datum/targetable/revenantAbility/help/cast(atom/target)
	if (..())
		return 1
	if (holder.help_mode)
		holder.help_mode = 0
