...
/datum/targetable/vampire/glare/cast(mob/target)
	if (!holder)
		return TRUE
	...
	E.color = "#FFFFFF"
	E.setup(M.loc)
	playsound(M.loc, 'sound/effects/glare.ogg', 50, TRUE, pitch = 1, extrarange = -4)

	SPAWN(1 DECI SECOND)
		var/obj/itemspecialeffect/glare/EE = new /obj/itemspecialeffect/glare
		EE.color = "#FFFFFF"
		EE.setup(target.loc)
		playsound(target.loc, 'sound/effects/glare.ogg', 50, TRUE, pitch = 0.8, extrarange = -4)
