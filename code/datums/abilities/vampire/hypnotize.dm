...
/datum/targetable/vampire/hypnotize/cast(mob/target)
	if (!holder)
		return TRUE
...
/datum/action/bar/icon/vamp_hypno/New(U,T,H)
	M = U
	target = T
	hypno = H
	..()

/datum/action/bar/icon/vamp_hypno/onUpdate()
	..()
...
/datum/action/bar/icon/vamp_hypno/onStart()
	..()
	if(hypno == null || GET_DIST(M, target) > hypno.max_range || M == null || target == null)
		interrupt(INTERRUPT_ALWAYS)
		boutput(M, "<span class='alert'>Your attempt to hypnotize the target was interrupted!</span>")
...
/datum/action/bar/icon/vamp_hypno/onEnd()
	..()
