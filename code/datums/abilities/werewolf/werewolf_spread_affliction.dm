...
/datum/targetable/werewolf/werewolf_spread_affliction/cast(mob/target)
	if (!holder)
		return TRUE
...
/datum/action/bar/private/icon/werewolf_spread_affliction/New(Target, Spread)
	target = Target
	spread = Spread
	..()

/datum/action/bar/private/icon/werewolf_spread_affliction/onStart()
	..()
	...
	if (target.stat == 2)
		...
		return
	A.locked = TRUE
	playsound(M.loc, pick('sound/voice/animal/werewolf_attack3.ogg'), 50, TRUE)
	M.visible_message("<span class='alert'><B>[M] lunges at [target]!</b></span>")

/datum/action/bar/private/icon/werewolf_spread_affliction/onUpdate()
	..()
	var/mob/living/M = owner
	var/datum/abilityHolder/A = spread.holder
	if (!spread || GET_DIST(M, target) > spread.max_range || target == null || M == null || !ishuman(target) || !ishuman(M) || !A || !istype(A))
...
/datum/action/bar/private/icon/werewolf_spread_affliction/onEnd()
	..()
	var/datum/abilityHolder/A = spread.holder
	var/mob/living/M = owner
...
/datum/action/bar/private/icon/werewolf_spread_affliction/onInterrupt()
	..()
	var/datum/abilityHolder/A = spread.holder
