var/list/animal_spell_critter_paths = list(
...
/datum/targetable/spell/animal/cast(mob/target)
	if (!holder)
		return TRUE
...
/datum/action/bar/polymorph/New(Source, Target, Spell)
	target = Target
	spell = Spell
	A = spell.holder
	M = Source
...
/datum/action/bar/polymorph/onStart()
	..()

	if (isnull(A) || GET_DIST(M, target) > spell.max_range || isnull(M) || !ishuman(target) || !M.wizard_castcheck(spell))
		interrupt(INTERRUPT_ALWAYS)

/datum/action/bar/polymorph/onUpdate()
	..()

	if (isnull(A) || GET_DIST(M, target) > spell.max_range || isnull(M) || !ishuman(target) || !M.wizard_castcheck(spell))
		interrupt(INTERRUPT_ALWAYS)

/datum/action/bar/polymorph/onEnd()
	..()
