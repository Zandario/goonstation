...
/datum/targetable/spell/cluwne/cast(mob/target)
	if(!holder)
		return TRUE
...
/datum/action/bar/icon/cluwne_spell/New(Source, Target, Spell)
	target = Target
	spell = Spell
	A = spell.holder
	M = Source
...
/datum/action/bar/icon/cluwne_spell/onStart()
	..()

	if (isnull(A) || GET_DIST(M, target) > spell.max_range || isnull(M) || !ishuman(target) || !M.wizard_castcheck(spell))
		interrupt(INTERRUPT_ALWAYS)

/datum/action/bar/icon/cluwne_spell/onUpdate()
	..()

	if (isnull(A) || GET_DIST(M, target) > spell.max_range || isnull(M) || !ishuman(target) || !M.wizard_castcheck(spell))
		interrupt(INTERRUPT_ALWAYS)

/datum/action/bar/icon/cluwne_spell/onEnd()
	..()
