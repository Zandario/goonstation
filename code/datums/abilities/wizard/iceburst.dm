...
/datum/targetable/spell/iceburst/cast()
	if(!holder)
		return
	var/count = 0
	var/count2 = 0
...
/obj/icecube/New(loc, mob/iced as mob)
	..()
...
/obj/icecube/relaymove(mob/user as mob)
	if (user.stat)
		return
...
/obj/icecube/proc/takeDamage(var/damage)
	src.health -= damage
	if(src.health <= 0)
		qdel(src)
		return
...
/obj/icecube/attackby(obj/item/W, mob/user)
	takeDamage(W.force)

/obj/icecube/mob_flip_inside(var/mob/user)
	..(user)
	user.show_text("<span class='alert'>[src] [pick("cracks","bends","shakes","groans")].</span>")
	src.takeDamage(6)

/obj/icecube/ex_act(severity)
