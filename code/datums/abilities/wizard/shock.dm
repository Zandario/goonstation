...
/datum/targetable/spell/shock/cast(mob/target)
	if(!holder)
		return
	if(!istype(target))
		return TRUE
	if(!IN_RANGE(target, holder.owner, max_range))
		return TRUE
	playsound(holder.owner.loc, 'sound/effects/elec_bzzz.ogg', 25, TRUE, -1)
	if(!istype(get_area(holder.owner), /area/sim/gunsim))
		holder.owner.say("EI NATH", FALSE, maptext_style, maptext_colors)
	..()

	playsound(holder.owner.loc, 'sound/effects/elec_bigzap.ogg', 25, TRUE, -1)
