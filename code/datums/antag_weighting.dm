...
/datum/antagWeighter/New(debugMode)
	..()
	src.debug = debugMode ? debugMode : 0


/datum/antagWeighter/proc/debugLog(msg)
	out(world, msg)
 ...
 */
/datum/antagWeighter/proc/history(role = "", ckey = "")
	if (!role || !ckey)
...
* @return list List of history details
*/
/datum/antagWeighter/proc/completeHistory(ckey = "")
	if (!ckey)
 ...
 */
/datum/antagWeighter/proc/simulateHistory(role = "", list/ckeyMinds = list())
 ...
 */
/datum/antagWeighter/proc/calculateWeightings(role = "", list/history = list())
	if (!role)
 ...
 */
/datum/antagWeighter/proc/choose(list/pool = list(), role = "", amount = 0, recordChosen = 0)
 ...
 * @return null
 */
/datum/antagWeighter/proc/record(role = "", ckey = "", latejoin = 0)
	if (!role || !ckey)
		throw EXCEPTION("Incorrect parameters given")
 ...
 *			"ckeyforadude1" = "traitor",
	...
	* @return null
	*/
/datum/antagWeighter/proc/recordMultiple(list/players = list())
	if (!players.len)
		throw EXCEPTION("Incorrect parameters given")
