...
/datum/apiHandler/proc/apiError(message = "", forceErrorException = 0)
 ...
 */
/datum/apiHandler/proc/retryApiQuery(list/givenArgs, attempt = 1)
 ...
 *
 */
/datum/apiHandler/proc/queryAPI(route = "", query = list(), forceResponse = 0, attempt = 1, forceErrorException = 0)
	if (!enabled || !route)
