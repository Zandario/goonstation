...
/datum/wage_system/proc/default_wages()
...
/datum/wage_system/proc/process()
	if(!ticker)
		return
...
/datum/wage_system/proc/checkLotteryTime()
...
/datum/wage_system/proc/start_lottery()
	src.time_until_lotto = ( ticker ? ticker.round_elapsed_ticks : 0 ) + time_between_lotto
	lottery_active = 1
	return

/datum/wage_system/proc/payday()
...
/datum/wage_system/proc/lotteryDay()
...
/*
/datum/wage_system/proc/update_wage(var/mob/living/carbon/C, var/rank)
...
/obj/machinery/computer/ATM
	...
	var/pin = null
/obj/machinery/computer/ATM/attackby(obj/item/I, mob/user)
...
/obj/machinery/computer/ATM/attack_ai(mob/user as mob)
	return
...
/obj/machinery/computer/bank_data/attack_ai(mob/user as mob)
	return src.Attackhand(user)

/obj/machinery/computer/bank_data/attackby(obj/item/I, mob/user)
	if (istype(I, /obj/item/card/id))
		if (!src.scan)
...
/obj/machinery/computer/bank_data/attack_hand(mob/user)
	if(..())
		return
	var/list/dat = list()
	if (src.temp)
...
/obj/machinery/computer/bank_data/Topic(href, href_list)
	if(..())
		return
...
/obj/submachine/ATM/attackby(obj/item/I, mob/user)
	if(broken)
...
/obj/submachine/ATM/attack_ai(mob/user as mob)
	return

/obj/submachine/ATM/attack_hand(mob/user)
	if(broken)
		boutput(user, "<span class='alert'>With its money removed and circuitry destroyed, it's unlikely this ATM will be able to do anything of use.</span>")
		return
	if(..())
...
/obj/submachine/ATM/bullet_act(obj/projectile/P)
...
/obj/submachine/ATM/proc/TryToFindRecord()
	if(src.scan)
		src.accessed_record = data_core.bank.find_record("name", src.scan.registered)
		return !!src.accessed_record
	return 0
...
/obj/submachine/ATM/Topic(href, href_list)
	if(..())
		return
	src.add_dialog(usr)
...
/obj/submachine/ATM/proc/take_damage(damage_amount = 5, mob/user as mob)
	if (broken)
		return
...
/obj/submachine/ATM/ex_act(severity)
	src.take_damage(70)
...
/obj/submachine/ATM/afterlife/take_damage(damage_amount = 5, mob/user as mob)
	return
...
/obj/item/lotteryTicket/disposing()
	. = ..()
	STOP_TRACKING

/proc/FindBankAccountByName(nametosearch)
