...
/datum/buildmode/New(datum/buildmode_holder/H)
	..()
	holder = H
	update_button_text()

/datum/buildmode/Read(F)
	. = ..()
	if(!isnull(src.hotkey_number))
		on_add_to_hotkey_bar()

/datum/buildmode/proc/copy()
...
/datum/buildmode/proc/deselected()
	for (var/atom/movable/screen/S in extra_buttons)
		holder.owner.screen -= S
...
/datum/buildmode/proc/resumed()
	for (var/atom/movable/screen/S in extra_buttons)
		holder.owner.screen += S

/datum/buildmode/proc/on_add_to_hotkey_bar()
...
/datum/buildmode/proc/paused()
	for (var/atom/movable/screen/S in extra_buttons)
		holder.owner.screen -= S

/datum/buildmode/proc/update_icon_state(newstate)
	icon_state = newstate
	holder.update_mode_icon()

/datum/buildmode/proc/update_button_text(new_text = "")
 ...
 */
/datum/buildmode/proc/click_raw(atom/object, location, control, params)

/datum/buildmode/proc/click_mode_right(ctrl, alt, shift)

/datum/buildmode/proc/click_left(atom/object, ctrl, alt, shift)

/datum/buildmode/proc/click_right(atom/object, ctrl, alt, shift)
...
/datum/buildmode_holder/New(var/client/C)
	..()
...
/datum/buildmode_holder/Read(F)
	. = ..()
	if(!isnull(src.hotkey_bar[1]))
		select_mode(src.hotkey_bar[1])

/datum/buildmode_holder/proc/set_client(client/C)
	owner = C
...
/datum/buildmode_holder/proc/select_mode(datum/buildmode/M)
	if (mode)
		mode.deselected()
	mode = M
	M.selected()
...
/datum/buildmode_holder/proc/number_key_pressed(numkey, keys_modifier)
	var/index = numkey
	if(index == 0)
		index = 10
...
/datum/buildmode_holder/proc/update_mode_icon()
...
/datum/buildmode_holder/proc/build_click(atom/target, location, control, list/params)
...
/datum/buildmode_holder/proc/activate()
	is_active = 1
...
/datum/buildmode_holder/proc/deactivate()
	is_active = 0
	owner.screen -= button_dir
	owner.screen -= button_help
	owner.screen -= button_mode
...
/datum/buildmode_holder/proc/add_to_hotkey_bar(datum/buildmode/mode, index=null)
	if(isnull(index))
...
/datum/buildmode_holder/proc/remove_from_hotkey_bar(datum/buildmode/mode)
...
/datum/buildmode_holder/proc/display_help()
...
/atom/movable/screen/buildmode/builddir/New(L, H)
	..()
	holder = H

/atom/movable/screen/buildmode/builddir/clicked(list/paramList)
...
/atom/movable/screen/buildmode/buildhelp/New(L, H)
	..()
	holder = H

/atom/movable/screen/buildmode/buildhelp/clicked(location, control, params)
	holder.display_help()
...
/atom/movable/screen/buildmode/buildquit/New(L, H)
	..()
	holder = H

/atom/movable/screen/buildmode/buildquit/clicked(location, control, params)
...
/atom/movable/screen/buildmode/buildmode/New(L, H)
	..()
	holder = H

/atom/movable/screen/buildmode/buildmode/clicked(list/pa)
	if ("middle" in pa)
...
/atom/movable/screen/buildmode/hotkey/New(L, H, datum/buildmode/mode)
	..()
	holder = H
	src.mode = mode

/atom/movable/screen/buildmode/hotkey/clicked(list/pa)
	if ("middle" in pa)
...
/atom/movable/screen/buildmode/hotkey/MouseWheel(delta_x, delta_y, location, control, params)
	. = ..()
	var/current = 0
