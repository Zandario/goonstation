...
/datum/character_preview/New(client/viewer, window_id, control_id = null)
	. = ..()
	START_TRACKING
...
/datum/character_preview/proc/add_background(color)
...
/datum/character_preview/disposing()
	STOP_TRACKING
	SPAWN(0)
		if (src.viewer)
...
/datum/character_preview/proc/update_appearance(datum/appearanceHolder/AH, datum/mutantrace/MR = null, direction = SOUTH, name = "human")
...
/datum/character_preview/window/New(client/viewer)
...
/datum/character_preview/window/disposing()
	. = ..()
	SPAWN(0)
		if (src.viewer)
...
/datum/character_preview/window/proc/show(shown = TRUE)
...
/datum/character_preview/multiclient/New(control_id = null)
	. = ..(null, "unused", control_id)

/datum/character_preview/multiclient/disposing()
...
/datum/character_preview/multiclient/add_background(color)
	. = ..()
	for(var/client/viewer in src.viewers)
		viewer.screen |= src.background

/// Adds a subscribed client
/datum/character_preview/multiclient/proc/add_client(client/viewer)
...
/datum/character_preview/multiclient/proc/remove_client(client/viewer)
	if (viewer && (viewer in src.viewers))
		src.viewers -= viewer
		viewer.screen -= src.handler
		viewer.screen -= src.preview_mob
...
/datum/character_preview/multiclient/proc/remove_all_clients()
	for (var/client/viewer in src.viewers)
		if (viewer)
			viewer.screen -= src.handler
			viewer.screen -= src.preview_mob
