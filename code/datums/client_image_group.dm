...
/datum/client_image_group/proc/add_image(image/img)
	src.images.Add(img)
...
/datum/client_image_group/proc/remove_image(image/img)
...
/datum/client_image_group/proc/add_mob(mob/added_mob)
...
/datum/client_image_group/proc/remove_mob(mob/removed_mob, force = FALSE) // same just reverse, and unregisters signals
	if (force)
...
/datum/client_image_group/proc/add_mind(datum/mind/added_mind)
...
/datum/client_image_group/proc/remove_mind(datum/mind/removed_mind)
...
/datum/client_image_group/proc/on_mind_attach(datum/mind/mind, mob/M)
	PRIVATE_PROC(TRUE)
	add_mob(M)

/// when a registered mind detaches from a mob
/datum/client_image_group/proc/on_mind_detach(datum/mind/mind, mob/M)
	PRIVATE_PROC(TRUE)
	remove_mob(M)

/// Registered on MOB_LOGIN, when a client enters the mob adds the images to it.
/datum/client_image_group/proc/add_images_to_client_of_mob(mob/target_mob)
	PRIVATE_PROC(TRUE)
	for (var/image/I as() in images)
...
/datum/client_image_group/proc/remove_images_from_client_of_mob(mob/target_mob)
	PRIVATE_PROC(TRUE)
...
/datum/client_image_group/proc/remove_mob_forced(mob/removed_mob)
	PRIVATE_PROC(TRUE)
	subscribed_mobs_with_subcount.Remove(removed_mob)
	UnregisterSignal(removed_mob, list(COMSIG_MOB_LOGIN, COMSIG_MOB_LOGOUT, COMSIG_PARENT_PRE_DISPOSING))

/// Removes or adds images to clients depending on the mob who the icons belong to's invisibility.
/datum/client_image_group/proc/on_mob_invisibility_changed(mob/invis_updated_mob)
	PRIVATE_PROC(TRUE)
