...
/datum/component/radioactive/Initialize(radStrength=100, decays=FALSE, neutron=FALSE, effectRange=1)
...
/datum/component/radioactive/proc/do_filters()
	var/atom/PA = parent
...
/datum/component/radioactive/proc/process()
	if(QDELETED(parent))
		global.processing_items.Remove(src)
		return
	ticked(parent)

/datum/component/radioactive/UnregisterFromParent()
	. = ..()
	var/atom/PA = parent
...
/datum/component/radioactive/InheritComponent(datum/component/radioactive/R, i_am_original)
	if (i_am_original)
...
/datum/component/radioactive/proc/ticked(atom/owner, mult=1)
	var/atom/PA = parent
...
/datum/component/radioactive/proc/touched(atom/owner, mob/toucher)
	if(istype(toucher))
...
/datum/component/radioactive/proc/eaten(atom/owner, mob/eater)
	if(istype(eater))
...
/datum/component/radioactive/proc/examined(atom/owner, mob/examiner, list/lines)
	var/rad_word = ""
	switch(radStrength)
		if(0 to 10)
