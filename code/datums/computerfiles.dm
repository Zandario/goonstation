...
/datum/computer/folder/proc/add_file(datum/computer/R)
...
/datum/computer/folder/proc/remove_file(datum/computer/R)
	if(holder && !holder.read_only && R)
...
/datum/computer/folder/proc/can_add_file(datum/computer/R)
	if(!holder || holder.read_only || !R)
		return 0
	if(istype(R,/datum/computer/folder) && (src.gen>=10))
		return 0
...
/datum/computer/folder/proc/copy_folder(depth = 0)
	if (depth >= 8)
		return null
...
/datum/computer/file/proc/copy_file_to_folder(datum/computer/folder/newfolder, newname)
...
/datum/computer/file/proc/copy_file() //Just make a replica of self
...
/datum/computer/proc/asText() //Convert contents to text, if possible
		return null
...
/datum/computer/file/archive/proc/add_file(datum/computer/R)
...
/datum/computer/file/archive/copy_file() //Just make a replica of self
...
/datum/computer/folder/link/New(datum/computer/folder/newtarget)
	..()
	if (gen != 10) gen = 10
	if(istype(newtarget))
...
/datum/computer/folder/link/add_file(datum/computer/R, misc)
	if (!target || target.holder != src.holder)
		return 0

	return target.add_file(R, misc)

/datum/computer/folder/link/can_add_file(datum/computer/R, misc)
	if (!target || target.holder != src.holder)
		return 0

	return target.can_add_file(R, misc)

/datum/computer/folder/link/remove_file(datum/computer/R, misc)
	if(!target || target.holder != src.holder)
		return 0

	return target.remove_file(R, misc)

/datum/computer/folder/link/copy_folder(var/depth = 0)
	if(!target || target.holder != src.holder)
		return 0
