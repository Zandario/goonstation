...
/datum/event_controller/proc/process()
...
/datum/event_controller/proc/event_cycle()
	event_cycle_count++
...
/datum/event_controller/proc/minor_event_cycle()
...
/datum/event_controller/proc/spawn_event(type = "player")
	var/do_event = 1
	if (!events_enabled)
...
/datum/event_controller/proc/do_random_event(list/event_bank, source = null)
...
/datum/event_controller/proc/force_event(string, reason)
	if (!string)
		return
	if (!reason)
...
/datum/event_controller/proc/event_config()
...
/datum/event_controller/Topic(href, href_list[])
