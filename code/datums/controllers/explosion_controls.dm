...
/datum/explosion_controller/proc/explode_at(atom/source, turf/epicenter, power, brisance = 1, angle = 0, width = 360, turf_safe=FALSE)
...
/datum/explosion_controller/proc/queue_damage(list/new_turfs, list/new_blame)
	var/c = 0
...
/datum/explosion_controller/proc/highest_explosion_power(obj/object)
	for (var/turf/T in object.locs)
		. = max(., queued_turfs[T])

/datum/explosion_controller/proc/kaboom()
...
/datum/explosion_controller/proc/process()
	if (exploding)
		return
...
/datum/explosion/New(atom/source, turf/epicenter, power, brisance, angle, width, user, turf_safe=FALSE)
	..()
	src.source = source
...
/datum/explosion/proc/logMe(power)
