...
/datum/job_controller
	...
	/// Not visible to players, for admin stuff, like the respawn panel.
	var/list/hidden_jobs = list()
	/// Hopefully this doesn't break anything!!
	var/allow_special_jobs = 1
...
/datum/job_controller/proc/job_config()
...
/// Since this is a 'public' window that everyone can get to, make sure we keep the user contained to their own savefile.
/datum/job_controller/proc/check_user_changed()
...
/datum/job_controller/proc/job_creator()
...
/datum/job_controller/Topic(href, href_list[])
	// JOB CONFIG COMMANDS
	USR_ADMIN_ONLY
...
/proc/find_job_in_controller_by_string(string, staple_only = 0)
...
/proc/find_job_in_controller_by_path(path)
...
/client/proc/cmd_job_controls()
	...
	if (job_controls == null)
		boutput(src, "UH OH! Shit's broken as fuck!")
	else
		src.debug_variables(job_controls)
