...
/datum/controller/process/delete_queue/setup()
	name = "DeleteQueue"
...
/datum/controller/process/delete_queue/doWork()
	/*
...
/datum/controller/process/delete_queue/proc/gimmick_ungcd_mob_stuff(mob/living/L)
...
/datum/controller/process/delete_queue/tickDetail()
	#ifdef DELETE_QUEUE_DEBUG
...
proc/ref_visit_list(list/L, list/next, datum/target, list/result, list/stack=null)
	var/i = 0
...
/proc/find_all_references_to(datum/D)
...
/proc/ref_visit_list_2(list/L, datum/target, list/result, list/stack=null)
	var/i = 0
	var/top_level = isnull(stack)
	if(isnull(stack))
		stack = list()
