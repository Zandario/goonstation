...
/proc/BeginSpacePush(atom/movable/A)
	if (!(A.temp_flags & SPACE_PUSHING))
		spacePushList += A
		A.temp_flags |= SPACE_PUSHING

/proc/EndSpacePush(atom/movable/A)
	if(ismob(A))
		var/mob/M = A
		M.inertia_dir = 0
	spacePushList -= A
...
/datum/controller/process/fMove/setup()
	name = "Forced movement"
	schedule_interval = 0.5 SECONDS

/datum/controller/process/fMove/doWork()
	//space first :)
...
/datum/controller/process/fMove/tickDetail()
