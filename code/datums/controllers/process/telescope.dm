...
/datum/controller/process/telescope/setup()
	name = "Telescope"
	schedule_interval = 10 SECONDS

/datum/controller/process/telescope/copyStateFrom(datum/controller/process/target)
	var/datum/controller/process/telescope/old_telescope = target
	src.manager = old_telescope.manager

/datum/controller/process/telescope/doWork()
	if(tele_man)
