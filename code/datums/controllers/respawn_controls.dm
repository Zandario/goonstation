...
/datum/respawn_controls/proc/process()
	if(!respawns_enabled)
		return
...
/datum/respawn_controls/proc/checkRespawnee(datum/respawnee/R)
	switch(R.checkValid())
...
/datum/respawn_controls/proc/subscribeNewRespawnee(ckey)
...
/datum/respawn_controls/proc/unsubscribeRespawnee(ckey)
	if(!ckey)
		return
	var/datum/respawnee/R = respawnees[ckey]
...
/datum/respawn_controls/proc/doRespawn(ckey)
	if(!ckey)
		return
	var/datum/respawnee/R = respawnees[ckey]
	if(R) R.doRespawn()
...
/datum/respawnee/disposing()
	the_client?.verbs -= /client/proc/respawn_via_controller
	master = null
	..()

/datum/respawnee/proc/initialize(ckey, datum/respawn_controls/master)
	src.ckey = ckey
...
/datum/respawnee/proc/update_time_display()
...
/datum/respawnee/proc/checkValid()
...
/datum/respawnee/proc/notifyAndGrantVerb()
...
/datum/respawnee/proc/doRespawn()
...
/atom/movable/screen/respawn_timer/proc/activate_clickability(rp=FALSE)
...
/atom/movable/screen/respawn_timer/proc/set_time_left(time)
	var/time_text
	if(time <= 75 SECONDS)
...
/atom/movable/screen/join_other/Topic(href, href_list)
	. = ..()
	if(href_list["action"] == "close")
		src.maptext= null

/atom/movable/screen/join_other/New(loc, server_id=null, server_name=null)
	..()
	if(!isnull(server_id))
