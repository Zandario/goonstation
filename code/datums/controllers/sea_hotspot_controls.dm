...
/datum/hotspot_controller/proc/generate_map()
	if (!map)
...
/datum/hotspot_controller/proc/generate_map_html()
	if (!src.map)
		// what the fuck???
		return
...
/datum/hotspot_controller/proc/show_map(client/C)
	if (!C)
		return
...
/datum/hotspot_controller/proc/clear()
	hotspot_groups.len = 0

/datum/hotspot_controller/proc/process() //do movety uptdate
	for (var/datum/sea_hotspot/S in hotspot_groups)
		S.drift_count++
...
/datum/hotspot_controller/proc/get_hotspot(turf/T)
	.= 0
	for (var/datum/sea_hotspot/S in hotspot_groups)
...
/datum/hotspot_controller/proc/get_hotspots_list(turf/T)
	.= list()
	for (var/datum/sea_hotspot/S in hotspot_groups)
		if(S.get_tile_heat(T))
			.+= S

/datum/hotspot_controller/proc/get_hotspots_amount(turf/T)
	.= 0
	for (var/datum/sea_hotspot/S in hotspot_groups)
		if(S.get_tile_heat(T))
			.+= 1

/datum/hotspot_controller/proc/probe_turf(turf/T)
	.= 0
	for (var/datum/sea_hotspot/S in hotspot_groups)
...
///t Triggered from asteroid mining currently.
/datum/hotspot_controller/proc/disturb_turf(turf/T)
	if (T.z != 5) return
...
/// Move hotspot 1 tile and set its dir to the difference between stomp loc and hotspot center.
/datum/hotspot_controller/proc/stomp_turf(turf/T)
	.= 0
	for (var/datum/sea_hotspot/S in hotspot_groups)
		if (S.get_tile_heat(T))
...
/datum/hotspot_controller/proc/colorping_at_turf(var/turf/T)
	for (var/datum/sea_hotspot/S in hotspot_groups)
		if(S.get_tile_heat(T))
			S.color_ping()
...
/datum/sea_hotspot
	...
	/// When mining underneath this hotspot on the trench zlevel, increase bonus heat (NOT USED)
	//var/static/max_activity_heat_bonus = 2000
	/// When polled at cap, return heat with this multiplier applied.
	//var/static/heat_polled_past_max_factor = 0.1
...
/datum/sea_hotspot/proc/move_center_to(turf/new_center)
...
/datum/sea_hotspot/proc/do_phenomena(recursion = 0, recursion_heat = 0)
...
/datum/sea_hotspot/proc/poll_capture_amt(turf/center)
	vent_capture_amt = 0
...
/datum/sea_hotspot/proc/get_tile_heat(turf/T)
...
/datum/sea_hotspot/proc/color_ping(setcolor = "#FF0011")
...
/datum/hotspot_point/New(x=0,y=0,z=0)
	..()
	src.x = x
	src.y = y
	src.z = z

/datum/hotspot_point/proc/change(x=0,y=0,z=0)
	src.x = x
	src.y = y
	src.z = z

/datum/hotspot_point/proc/turf()
	.= locate(x,y,z)
...
/obj/item/heat_dowsing
	...
	/// Speak every [x] process ticks (machine loop targets about 1 tick per 2 seconds)
	var/static/speak_interval = 8
	var/speak_count = 8
...
/obj/item/heat_dowsing/New()
	..()
	src.setItemSpecial(/datum/item_special/swipe)

/obj/item/heat_dowsing/ex_act(severity)
	return //nah

/obj/item/heat_dowsing/disposing()
	deployed = 0
	closest_hotspot = 0
	processing_items -= src
	..()

/obj/item/heat_dowsing/process()
...
/obj/item/heat_dowsing/attackby(obj/item/I, mob/M)
	if (ispryingtool(I))
		if (deployed)
			src.undeploy()
		else
...
/obj/item/heat_dowsing/attack_hand(mob/living/carbon/human/user)
	src.undeploy()
	..()

/obj/item/heat_dowsing/afterattack(turf/T, mob/user)
...
/obj/item/heat_dowsing/proc/undeploy()
	src.icon_state = "dowsing_hands"
	deployed = 0
	closest_hotspot = 0
	processing_items -= src

/obj/item/heat_dowsing/proc/deploy()
	processing_items |= src
...
/turf/space/fluid/attack_hand(mob/user)
	var/obj/item/heat_dowsing/H = locate() in src
	if (H)
		H.Attackhand(user)

/turf/space/fluid/attackby(obj/item/W, mob/user)
...
/obj/venthole/ex_act(severity)
	return //nah

/obj/venthole/get_desc(dist)
	if (dist <= 4)
...
/obj/venthole/attackby(obj/item/W, mob/user)
...
/obj/item/vent_capture_unbuilt/attackby(obj/item/W, mob/user)
	if (istype(W,/obj/item/electronics/soldering) || isscrewingtool(W) || ispryingtool(W) || iswrenchingtool(W))
		build(user,src.loc)
		return
	..()

/obj/item/vent_capture_unbuilt/attack_self(mob/user)
	build(user,user.loc)

/obj/item/vent_capture_unbuilt/proc/build(mob/user, turf/build_turf)
...
/obj/item/vent_capture_unbuilt/proc/finish_build(turf/T)
...
/obj/machinery/power/vent_capture/New()
	..()
	START_TRACKING
...
/obj/machinery/power/vent_capture/ex_act(severity)
	return //nah

/obj/machinery/power/vent_capture/update_icon()
	icon_state = icon_state = "hydrovent_[built]"

/obj/machinery/power/vent_capture/disposing()
	..()
	STOP_TRACKING
	if (istype(src.loc,/turf/space/fluid))
		var/turf/space/fluid/T = src.loc
...
/obj/machinery/power/vent_capture/attackby(obj/item/W, mob/user)
	if (istype(W,/obj/item/electronics/soldering) || isscrewingtool(W) || ispryingtool(W) || iswrenchingtool(W))
		actions.start(new/datum/action/bar/icon/unbuild_vent_capture(src), user)
		return
	// This was in another attackby for the same object, but I'm not sure why it was seperated, recommend someone else check this. @Zandario
	if (!built)
...
/obj/machinery/power/vent_capture/proc/unbuild(mob/user)
	new /obj/item/vent_capture_unbuilt(src.loc)
	qdel(src)

/obj/machinery/power/vent_capture/Move(NewLoc,Dir=0,step_x=0,step_y=0)
	if (istype(src.loc,/turf/space/fluid))
		var/turf/space/fluid/T = src.loc
		T.captured = 0
		update_capture()
...
/obj/machinery/power/vent_capture/proc/update_capture()
	if (!isturf(src.loc))
		return
...
/obj/machinery/power/vent_capture/process()
	if(status & BROKEN)
		return
	if (!built) return
...
/obj/machinery/power/vent_capture/get_desc(dist)
	if (!built)
...
/*
/obj/machinery/power/vent_capture/proc/healthcheck()
	if (src.health <= 0)
		if(!(status & BROKEN))
			broken()
		else
...
/obj/machinery/power/stomper/New()
	..()
	cell = new(src)
	cell.charge = 1000
	cell.maxcharge = 1000

/obj/machinery/power/stomper/ex_act(severity)
	return //nah

/obj/machinery/power/stomper/emag_act(mob/user, obj/item/card/emag/E)
	if (src.emagged)
...
/obj/machinery/power/stomper/update_icon()
	icon_state = "stomper[on]"

/obj/machinery/power/stomper/attack_hand(mob/living/carbon/human/user)
...
/obj/machinery/power/stomper/proc/activate()
	on = !on
	if (set_anchor)
		anchored = on
...
/obj/machinery/power/stomper/verb/toggle()
...
/obj/machinery/power/stomper/attackby(obj/item/I, mob/user)
...
/obj/item/clothing/shoes/stomp_boots/setupProperties()
	. = ..()
...
/obj/ability_button/stomper_boot_stomp/execute_ability()
...
/datum/action/bar/icon/build_vent_capture/New(Vent, Loc)
	V = Vent
	T = Loc
	..()

/datum/action/bar/icon/build_vent_capture/onUpdate()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || V == null || owner == null || T == null || V.loc != T)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/build_vent_capture/onStart()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || V == null || owner == null || T == null || V.loc != T)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/build_vent_capture/onEnd()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || V == null || owner == null || T == null || V.loc != T)
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/unbuild_vent_capture/New(Vent)
	V = Vent
	..()

/datum/action/bar/icon/unbuild_vent_capture/onUpdate()
	..()
	if(BOUNDS_DIST(owner, V) > 0 || V == null || owner == null)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/unbuild_vent_capture/onStart()
	..()
	if(BOUNDS_DIST(owner, V) > 0 || V == null || owner == null)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/unbuild_vent_capture/onEnd()
	..()
	if(BOUNDS_DIST(owner, V) > 0 || V == null || owner == null)
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/dig_sea_hole/New(Turf)
	T = Turf
	..()
	playsound(T, 'sound/effects/shovel1.ogg', 50, 1, 0.3)

/datum/action/bar/icon/dig_sea_hole/onUpdate()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || T == null)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/dig_sea_hole/onStart()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || T == null)
		interrupt(INTERRUPT_ALWAYS)
		return

/datum/action/bar/icon/dig_sea_hole/onEnd()
	..()
	if(BOUNDS_DIST(owner, T) > 0 || T == null)
		interrupt(INTERRUPT_ALWAYS)
		return
...
/datum/action/bar/icon/dig_sea_hole/fast
	duration = 5
...
/obj/item/trench_map/examine(mob/user)
...
/obj/item/trench_map/attack_self(mob/user)
	. = ..()
	src.examine(user)

/obj/item/trench_map/attack_hand(mob/user)
	if (!src.anchored)
		return ..()
...
/obj/item/trench_map/afterattack(atom/A as mob|obj|turf, mob/user as mob)
