...
/datum/controller/gameticker/proc/generate_crew_objectives()
	set background = 1
...
/datum/controller/gameticker/proc/generate_individual_objectives(datum/mind/crewMind)
	set background = 1
...
/datum/objective/crew/captain/drunk/check_completion()
...
/datum/objective/crew/headofsecurity/hat/check_completion()
...
/datum/objective/crew/headofsecurity/brig/check_completion()
...
/datum/objective/crew/headofsecurity/centcom/check_completion()
	if(isnull(check_result))
		check_result = FALSE
		for(var/datum/mind/M in ticker.minds)
...
/datum/objective/crew/headofsecurity/brigstir/check_completion()
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/headofpersonnel/vanish/check_completion()
...
/datum/objective/crew/chiefengineer/furnaces/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/chiefengineer/ptl/check_completion()
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/chiefengineer/reserves/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/chiefengineer/apc/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/securityofficer/centcom/check_completion()
	if(isnull(check_result))
		check_result = FALSE
		for(var/datum/mind/M in ticker.minds)
			if(M.special_role && M.current && !isobserver(M.current) && in_centcom(M.current) && M.current.hasStatus("handcuffed"))
...
/datum/objective/crew/securityofficer/brigstir/check_completion()
	if(isnull(check_result))
		check_result = FALSE
		for(var/mob/living/carbon/human/npc/monkey/stirstir/M in mobs)
			if(!isdead(M) && (M.get_brute_damage() + M.get_oxygen_deprivation() + M.get_burn_damage() + M.get_toxin_damage()) == 0 && istype(get_area(M),/area/station/security/brig))
...
/datum/objective/crew/securityassistant/brigstir/check_completion()
	if(isnull(check_result))
		check_result = FALSE
		for(var/mob/living/carbon/human/npc/monkey/stirstir/M in mobs)
			if(!isdead(M) && (M.get_brute_damage() + M.get_oxygen_deprivation() + M.get_burn_damage() + M.get_toxin_damage()) == 0 && istype(get_area(M),/area/station/security/brig))
...
/datum/objective/crew/quartermaster/profit/check_completion()
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/quartermaster/specialorder/check_completion()
...
/datum/objective/crew/detective/drunk/check_completion()
	if(owner.current && owner.current.reagents && owner.current.reagents.has_reagent("ethanol"))
		return 1
	else
		return 0
...
/datum/objective/crew/detective/gear/check_completion()
...
/datum/objective/crew/detective/smoke/check_completion()
...
/datum/objective/crew/botanist/mutantplants/check_completion()
	var/mutcount = 0
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/botanist/noweed/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/chaplain/funeral/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/janitor/cleanbar/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/janitor/cleanmedbay/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/janitor/cleanbrig/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/bartender/shotgun/check_completion()
...
/datum/objective/crew/bartender/drinks/set_up()
	..()
...
/datum/objective/crew/bartender/drinks/check_completion()
...
/datum/objective/crew/chef/cake/set_up()
	..()
...
/datum/objective/crew/chef/cake/check_completion()
	return completed
...
/datum/objective/crew/chef/pizza/set_up()
	..()
...
/datum/objective/crew/chef/pizza/check_completion()
	return completed
...
/datum/objective/crew/engineer/furnaces/check_completion()
	if(isnull(check_result))
		check_result = TRUE
		for(var/obj/machinery/power/furnace/F in machine_registry[MACHINES_POWER])
			if(F.z == Z_LEVEL_STATION && !F.active && istype(F.loc.loc, /area/station))
...
/datum/objective/crew/engineer/reserves/check_completion()
	if(isnull(check_result))
		check_result = TRUE
		for (var/obj/machinery/power/smes/S in machine_registry[MACHINES_POWER])
			if(istype(get_area(S),/area/station) && S.charge < S.capacity/5)
...
/datum/objective/crew/engineer/apc/check_completion()
	if(isnull(check_result))
		check_result = TRUE
		for_by_tcl(A, /obj/machinery/power/apc)
			if(istype(get_area(A),/area/station) && A.area.requires_power)
...
/datum/objective/crew/miner/isa/check_completion()
	var/suitcount = 0
	if(isnull(check_result))
...
/datum/objective/crew/miner/forsale/check_completion()
...
/datum/objective/crew/mechanic/scanned/check_completion()
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/mechanic/teleporter/check_completion()
	var/telecount = 0
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/researchdirector/heisenbee/check_completion()
...
/datum/objective/crew/researchdirector/noscorch/check_completion()
	if(isnull(check_result))
		check_result = TRUE
...
/datum/objective/crew/researchdirector/hyper/check_completion()
...
/datum/objective/crew/researchdirector/void/check_completion()
...
/datum/objective/crew/researchdirector/onfire/check_completion()
...
/datum/objective/crew/scientist/noscorch/check_completion()
	if(isnull(check_result))
		check_result = TRUE
		for(var/turf/simulated/floor/T in get_area_turfs(/area/station/science/chemistry, 0))
			if(T.burnt == 1)
...
/datum/objective/crew/scientist/hyper/check_completion()
	if(owner.current && owner.current.reagents && owner.current.reagents.has_reagent("methamphetamine"))
		return 1
	else
		return 0
...
/datum/objective/crew/scientist/void/check_completion()
	for_by_tcl(F, /obj/dfissure_to)
		if(F.z == Z_LEVEL_STATION) return 1
	return 0
...
/datum/objective/crew/scientist/onfire/check_completion()
	if(owner.current && !isdead(owner.current) && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
		if(in_centcom(H) && H.getStatusDuration("burning") > 1 && owner.current.reagents.has_reagent("silver_sulfadiazine")) return 1
		else return 0
...
/datum/objective/crew/medicaldirector/dr_acula/check_completion()
...
/datum/objective/crew/medicaldirector/dr_acula_feeds/check_completion()
	for (var/obj/critter/bat/doctor/Dr in by_cat[TR_CAT_PETS])
...
/datum/objective/crew/medicaldirector/headsurgeon/check_completion()
...
/datum/objective/crew/medicaldirector/scanned/check_completion()
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/medicaldirector/cyborgs/check_completion()
	var/borgcount = 0
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/medicaldirector/medibots/check_completion()
	var/medbots = 0
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/medicaldirector/buttbots/check_completion()
	var/buttbots = 0
	if(isnull(check_result))
		check_result = FALSE
...
/datum/objective/crew/medicaldirector/cryo/check_completion()
	if(isnull(check_result))
		var/cryocount = 0
		check_result = FALSE
...
/datum/objective/crew/medicaldirector/healself/check_completion()
...
/datum/objective/crew/medicaldirector/heal/check_completion()
...
/datum/objective/crew/medicaldirector/oath/check_completion()
...
/datum/objective/crew/geneticist/scanned/check_completion()
	if(isnull(check_result))
		check_result = FALSE
		for(var/obj/machinery/computer/cloning/C as anything in machine_registry[MACHINES_CLONINGCONSOLES])
			if(C.records.len > 4)
...
/datum/objective/crew/geneticist/booth/check_completion()
	if(isnull(check_result))
...
/datum/objective/crew/roboticist/medibots/check_completion()
	var/medbots = 0
	if(isnull(check_result))
		check_result = FALSE
		for (var/obj/machinery/bot/medbot/M in machine_registry[MACHINES_BOTS])
...
/datum/objective/crew/roboticist/buttbots/check_completion()
	var/buttbots = 0
	if(isnull(check_result))
		check_result = FALSE
		for(var/obj/machinery/bot/buttbot/B in machine_registry[MACHINES_BOTS])
...
/datum/objective/crew/medicaldoctor/cryo/check_completion()
	if(isnull(check_result))
		var/cryocount = 0
		check_result = FALSE
		for(var/obj/machinery/atmospherics/unary/cryo_cell/C in by_cat[TR_CAT_ATMOS_MACHINES])
...
/datum/objective/crew/medicaldoctor/healself/check_completion()
	if(owner.current && !isdead(owner.current) && (owner.current.get_brute_damage() + owner.current.get_oxygen_deprivation() + owner.current.get_burn_damage() + owner.current.get_toxin_damage()) == 0)
		return 1
	else
		return 0
...
/datum/objective/crew/medicaldoctor/heal/check_completion()
	if(patchesused > 9) return 1
	else return 0
...
/datum/objective/crew/medicaldoctor/oath/check_completion()
	if (owner?.violated_hippocratic_oath)
		return 0
	else
		return 1
...
/datum/objective/crew/staffassistant/butt/check_completion()
	if(owner.current && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/staffassistant/wearbutt/check_completion()
	if(owner.current && !isdead(owner.current) && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/staffassistant/promotion/check_completion()
	if(owner.current && !isdead(owner.current) && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/staffassistant/clown/check_completion()
	if(owner.current && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/staffassistant/chompski/check_completion()
...
/datum/objective/crew/staffassistant/mailman/check_completion()
	if(owner.current && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/staffassistant/spacesuit/check_completion()
	if(owner.current)
...
/datum/objective/crew/staffassistant/monkey/check_completion()
...
/datum/objective/crew/staffassistant/headsurgeon/check_completion()
	for (var/obj/H in by_cat[TR_CAT_HEAD_SURGEON])
		if (in_centcom(H))
			return 1
	return 0
...
/datum/objective/crew/technicalassistant/wearbutt/check_completion()
	if(owner.current && !isdead(owner.current) && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/objective/crew/technicalassistant/mailman/check_completion()
	if(owner.current && ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
		if(istype(H.w_uniform,/obj/item/clothing/under/misc/mail) || istype(H.head,/obj/item/clothing/head/mailcap)) return 1
	else return 0
...
/datum/objective/crew/technicalassistant/promotion/check_completion()
...
/datum/objective/crew/technicalassistant/spacesuit/check_completion()
	if(owner.current)
		for(var/obj/item/clothing/suit/space/S in owner.current.contents)
			return 1
	return 0
...
/datum/objective/crew/medicalassistant/monkey/check_completion()
	if(owner.current && !isdead(owner.current) && in_centcom(owner.current) && ismonkey(owner.current)) return 1
	else return 0
...
/datum/objective/crew/medicalassistant/promotion/check_completion()
	if(owner.current && !isdead(owner.current) && in_centcom(owner.current)) //checking basic stuff - they escaped alive and have an ID
		var/mob/living/carbon/human/H = owner.current
		if(H.wear_id && H.wear_id:registered == H.real_name && !(H.wear_id:assignment in list("Technical Assistant","Staff Assistant","Medical Assistant"))) return 1
		else return 0
...
/datum/objective/crew/medicalassistant/healself/check_completion()
	if(owner.current && !isdead(owner.current) && (owner.current.get_brute_damage() + owner.current.get_oxygen_deprivation() + owner.current.get_burn_damage() + owner.current.get_toxin_damage()) == 0)
		return 1
	else
		return 0
...
/datum/objective/crew/medicalassistant/headsurgeon/check_completion()
	for (var/obj/H in by_cat[TR_CAT_HEAD_SURGEON])
		if (in_centcom(H))
			return TRUE
	return 0
