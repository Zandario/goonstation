...
/datum/healthHolder/suffocation/proc/lose_breath(amt)
	losebreath = max(0, losebreath + amt)

/datum/healthHolder/suffocation/proc/gain_breath(amt)
	lose_breath(-amt)


/// Possibly not needed with new lifeprocess - didnt test tho lol
/datum/healthHolder/suffocation/on_life()
...
/datum/healthHolder/suffocation/prevents_speech()
	return losebreath
