...
/datum/datalogger/proc/Increment(p)
	if(!(p in stats))
		return null
	stats[p]++
...
/datum/datalogger/proc/Decrement(p)
	if(!(p in stats))
		return null
	stats[p]--
...
/datum/datalogger/proc/SetValue(p, val)
	if(!(p in stats))
		return null
	stats[p] = val
	return 1

/datum/datalogger/proc/ScanText(msg)
...
/datum/datalogger/proc/GetStat(index)
	if(!(index in stats))
		return null
	return stats[index]

/datum/datalogger/proc/AddDeath(mobName, mobCkey, where, health)
