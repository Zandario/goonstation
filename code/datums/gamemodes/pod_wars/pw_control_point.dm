/datum/control_point
	...
	/// Values from -100 to 100. Positives denote NT, negatives denote SY.  	/////////UNUSED
	var/capture_value = 0
	/// 1 or 3 based on if a commander has entered their code.  				/////////UNUSED
	var/capture_rate = 1
	/// 0 if not moving, either uncaptured or at max capture. 1=NT, 2=SY  	/////////UNUSED
	var/capturing_team
	/// 1=NT, 2=SY, not the team datum
	var/owner_team = 0
	/// Backend name, var/name is the user readable name. Used for warp beacon searching, etc.
	var/true_name
	/// Time it was last captured.
	var/last_cap_time
	/// var 0-3 none/low/med/high. Should correlate to holding the point for <5 min, <10 min, <15
	var/crate_rewards_tier = 0
	var/datum/game_mode/pod_wars/mode

/datum/control_point/New(obj/control_point_computer/computer, area/capture_area, name, true_name, datum/game_mode/pod_wars/mode)
	..()
...
/datum/control_point/proc/do_item_delivery()
	if (!src.computer)
...
/datum/control_point/proc/capture(mob/user, team_num)
...
/*
/datum/control_point/proc/receive_prevent_capture(var/mob/user, var/user_team)
	capturing_team = 0
	return

/datum/control_point/proc/capture_start(var/mob/user, var/user_team)
...
/datum/control_point/proc/process()
