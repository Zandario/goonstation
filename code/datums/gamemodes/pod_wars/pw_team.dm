...
/datum/pod_wars_team/New(datum/game_mode/pod_wars/mode, team_num)
	..()
	src.mode = mode
...
/datum/pod_wars_team/proc/change_points(amt)
	points += amt
	mode.handle_point_change(src)


/datum/pod_wars_team/proc/set_comms(datum/game_mode/pod_wars/mode)
...
/datum/pod_wars_team/proc/accept_initial_players(list/players)
	members = players
	if (!select_commander())
...
/datum/pod_wars_team/proc/select_commander()
...
/datum/pod_wars_team/proc/get_possible_commanders(priority)
...
/datum/pod_wars_team/proc/equip_player(mob/M, show_popup = FALSE)
...
/datum/pod_wars_team/proc/setup_voice_line_alt_amounts()
	switch(team_num)
		if (TEAM_NANOTRASEN)
