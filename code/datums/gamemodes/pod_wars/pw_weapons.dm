...
/obj/item/gun/energy/blaster_pod_wars/shoot(target, start, mob/user)
	if (canshoot())
		if (team_num)
...
/obj/item/gun/energy/blaster_pod_wars/shoot_point_blank(atom/target, mob/user, second_shot)
	if (canshoot())
		if (team_num)
			if (team_num == get_pod_wars_team_num(user))
				return ..(target, user)
...
/obj/item/gun/energy/blaster_pod_wars/disposing()
	indicator_display = null
	..()


/obj/item/gun/energy/blaster_pod_wars/New()
...
/obj/item/gun/energy/blaster_pod_wars/update_icon()
	..()
	// src.overlays = null
	var/list/ret = list()
...
/obj/item/gun/energy/blaster_pod_wars/proc/stop_charging()
...
/obj/item/gun/energy/blaster_pod_wars/nanotrasen
...
/obj/item/old_grenade/energy_frag/prime()
	var/turf/T = ..()
	if (T)
...
/obj/item/old_grenade/energy_concussion/prime()
	var/turf/T = ..()
	if (T)
