...
/datum/hud/drone/clear_master()
	master = null
	..()

/datum/hud/drone/relay_click(id)
	if (!master)
		return
	switch (id)
		if ("tool1")
...
/datum/hud/drone/proc/update_health()
	if (!health)
		return
	if (isdead(master))
...
/datum/hud/drone/proc/update_charge()
	if (!charge)
		return
	if (master.cell)
...
/datum/hud/drone/proc/set_active_tool()
	var/list_counter = 0
...
/datum/hud/drone/proc/update_tools()
...
/datum/hud/drone/proc/update_status_effects()
...
/mob/living/silicon/drone/updateStatusUi()
