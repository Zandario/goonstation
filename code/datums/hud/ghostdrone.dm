...
/datum/hud/ghostdrone
	...
	var/atom/movable/screen/hud/face
	...
	var/atom/movable/screen/hud/prevbg
	var/atom/movable/screen/hud/nextbg
	var/atom/movable/screen/hud/closebg
...
/datum/hud/ghostdrone/clear_master()
	master = null
	..()


/datum/hud/ghostdrone/proc/toggle_equipment()
	show_items = !show_items
	update_equipment()

/datum/hud/ghostdrone/proc/update_equipment()
...
/datum/hud/ghostdrone/proc/try_equip_at(var/i)
...
/datum/hud/ghostdrone/scrolled(id, dx, dy, user, parms, atom/movable/screen/hud/scr)
...
/datum/hud/ghostdrone/relay_click(id)
	if (!master)
		return
	switch (id)
		if ("next", "nextbg")
...
/datum/hud/ghostdrone/proc/set_active_tool(active) // naming these tools to distinuish it from the module of a borg
	mod1.icon_state = "toolslot[active ? 1 : ""]"

/datum/hud/ghostdrone/proc/update_tools()
...
/datum/hud/ghostdrone/proc/update_charge()
	if (master.cell)
...
/datum/hud/ghostdrone/proc/update_health()
	if (!isdead(master))
...
/datum/hud/ghostdrone/proc/update_pulling()
	pulling.icon_state = "pull[master.pulling ? 1 : 0]"

/datum/hud/ghostdrone/proc/update_environment()
...
/datum/hud/ghostdrone/proc/update_ability_hotbar()
	if (!master.client)
		return
	if(isdead(master))
		return
...
/datum/hud/ghostdrone/proc/update_status_effects()
...
/mob/living/silicon/ghostdrone/updateStatusUi()
