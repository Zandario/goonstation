/datum/hud/human
	...
	var/atom/movable/screen/hud/invtoggle
	var/atom/movable/screen/hud/belt
	var/atom/movable/screen/hud/storage1
	var/atom/movable/screen/hud/storage2
	var/atom/movable/screen/hud/back
	var/atom/movable/screen/hud/lhand
	var/atom/movable/screen/hud/rhand
	var/atom/movable/screen/hud/twohandl
	var/atom/movable/screen/hud/twohandr
	...
	var/atom/movable/screen/hud/sprinting
	var/atom/movable/screen/hud/swaphands
	var/atom/movable/screen/hud/equip
	var/atom/movable/screen/hud/tg_butts

	var/atom/movable/screen/hud/health
	var/atom/movable/screen/hud/health_brute
	var/atom/movable/screen/hud/health_burn
	var/atom/movable/screen/hud/health_tox
	var/atom/movable/screen/hud/health_oxy
	...
	var/atom/movable/screen/hud/ability_toggle
	var/atom/movable/screen/hud/stats
	var/atom/movable/screen/hud/legend
	var/atom/movable/screen/hud/sel
...
/datum/hud/human/proc/update_status_effects()
...
/datum/hud/human/clear_master()
	master = null
	..()

/datum/hud/human/New(M)
	..()
	if(isnull(M))
...
/datum/hud/human/relay_click(id, mob/user, list/params)
	switch (id)
		if ("invtoggle")
...
/datum/hud/human/MouseEntered(atom/movable/screen/hud/H, location, control, params)
	if (!H || usr != src.master)
		return
	var/obj/item/W = null
	var/obj/item/I
...
/datum/hud/human/MouseExited(atom/movable/screen/hud/H)
	if (!H || usr != src.master)
		return
	if (sel)
		sel.screen_loc = null
...
/datum/hud/human/MouseDrop(atom/movable/screen/hud/H, atom/over_object, src_location, over_location, over_control, params)
	if (!H)
		return
	var/obj/item/W = null
...
/datum/hud/human/MouseDrop_T(atom/movable/screen/hud/H, atom/movable/O as obj, mob/user as mob)
	if (!H)
		return
	var/obj/item/W = null
...
/datum/hud/human/proc/add_other_object(obj/item/I, loc) // this is stupid but necessary
...
/datum/hud/human/proc/remove_item(obj/item/I)
	if (length(inventory_items))
		inventory_items -= I
	remove_object(I)

/datum/hud/human/proc/update_hands()
...
/datum/hud/human/proc/update_stats()
	var/newDesc = ""
...
/datum/hud/human/proc/update_throwing()
	if (!throwing)
		return 0
...
/datum/hud/human/proc/update_intent()
	if (!intent)
		return 0
...
/datum/hud/human/proc/update_mintent()
	if (!mintent)
		return 0
	mintent.icon_state = "move-[master.m_intent]"

/datum/hud/human/proc/update_pulling()
	if (!pulling)
		return 0
	pulling.icon_state = "pull[!!master.pulling]"

/datum/hud/human/proc/update_resting()
	if (!resting)
		return 0
	resting.icon_state = "rest[master.hasStatus("resting") ? 1 : 0]"

/datum/hud/human/proc/update_ability_hotbar()
...
/datum/hud/human/proc/update_sprinting()
	if (!sprinting || !master.client)
		return 0
	sprinting.icon_state = "sprint[master.client.check_key(KEY_RUN)]"

/datum/hud/human/proc/update_indicators()
	update_health_indicator()
	update_blood_indicator()
	update_temp_indicator()

/datum/hud/human/proc/update_health_indicator()
	if (!health)
		return
...
/datum/hud/human/proc/update_blood_indicator()
	if (!src.bleeding)
		return //doesn't have a hud element to update
	if (isdead(master))
...
/datum/hud/human/proc/update_temp_indicator()
	if (!bodytemp)
		return
	if(master.getStatusDuration("burning") && !master.is_heat_resistant())
...
/datum/hud/human/proc/update_tox_indicator(status)
	if (!toxin)
		return
	toxin.icon_state = "tox[status]"

/datum/hud/human/proc/update_oxy_indicator(status)
	if (!oxygen)
		return
	oxygen.icon_state = "oxy[status]"

/datum/hud/human/proc/update_fire_indicator(status)
	if (!fire)
		return
	fire.icon_state = "fire[status]"

/datum/hud/human/proc/update_rad_indicator(status)
	if (!rad) // not rad :'(
		return
	rad.icon_state = "rad[status]"

/datum/hud/human/proc/change_hud_style(icon/new_file)
	if (new_file)
...
/datum/hud/human/proc/set_sprint(on)
	if(stamina)
...
/mob/proc/moused_over(obj/item/I)
	.=0

/mob/proc/moused_exit(obj/item/I)
	.=0


/mob/living/carbon/human/updateStatusUi()
	if(src.hud && istype(src.hud, /datum/hud/human))
		var/datum/hud/human/H = src.hud
		H.update_status_effects()
	return

/mob/living/carbon/human/moused_over(obj/item/I)
...
/mob/living/carbon/human/moused_exit(obj/item/I)
	if (src.client && src.client.hand_ghosts)
