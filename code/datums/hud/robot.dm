/atom/movable/screen/hud/robotstorage/MouseEntered(location, control, params)
	if (src.name)
		src.maptext_x = 34
		src.maptext_width = 128
		src.maptext = "<span class='vm l pixel sh'>[src.name]</span>"

/atom/movable/screen/hud/robotstorage/MouseExited(location, control, params)
	src.maptext = null

/datum/hud/silicon/robot
	var/atom/movable/screen/hud
	var/atom/movable/screen/hud/mod1
	var/atom/movable/screen/hud/mod2
	var/atom/movable/screen/hud/mod3
	var/atom/movable/screen/hud/charge
	var/atom/movable/screen/hud/module
	var/atom/movable/screen/hud/intent
	var/atom/movable/screen/hud/pulling

	var/atom/movable/screen/hud/prev
	var/atom/movable/screen/hud/boxes
	var/atom/movable/screen/hud/next
	var/atom/movable/screen/hud/close

	var/atom/movable/screen/hud/health
	var/atom/movable/screen/hud/oxy
	var/atom/movable/screen/hud/temp
...
/datum/hud/silicon/robot/clear_master()
	master = null
	..()


/datum/hud/silicon/robot/proc/toggle_equipment()
	show_items = !show_items
	update_equipment()

/datum/hud/silicon/robot/proc/module_added()
	items_screen = 1
	update_equipment()
	update_module()

/datum/hud/silicon/robot/proc/module_removed()
	items_screen = 1
	update_equipment()
	update_module()

/datum/hud/silicon/robot/proc/update_equipment()
...
/datum/hud/silicon/robot/proc/try_equip_at(var/i)
...
/datum/hud/silicon/robot/scrolled(id, dx, dy, user, parms, atom/movable/screen/hud/scr)
...
/datum/hud/silicon/robot/relay_click(id)
	if (!master)
		return
	switch (id)
		if ("next")
...
/datum/hud/silicon/robot/proc/update_status_effects()
...
/datum/hud/silicon/robot/update_health()
	..()
	if (!isdead(master))
...
/datum/hud/silicon/robot/proc/set_active_tool(active) // naming these tools to distinuish it from the module of a borg
	mod1.icon_state = "mod1[active == 1]"
	mod2.icon_state = "mod2[active == 2]"
	mod3.icon_state = "mod3[active == 3]"

/datum/hud/silicon/robot/proc/set_show_upgrades(show)
...
/datum/hud/silicon/robot/proc/update_tools()
...
/datum/hud/silicon/robot/proc/update_module()
	if (master.module)
...
/datum/hud/silicon/robot/proc/update_intent()
	intent.icon_state = "intent-[master.a_intent]"

/datum/hud/silicon/robot/proc/update_charge()
	if (master.cell)
		var/pct = round(100*master.cell.charge/master.cell.maxcharge, 1)
...
/datum/hud/silicon/robot/proc/maptext_health_percent(obj/item/parts/robot_parts/part)
...
/datum/hud/silicon/robot/proc/update_pulling()
	pulling.icon_state = "pull[master.pulling ? 1 : 0]"

/datum/hud/silicon/robot/proc/update_environment()
...
/datum/hud/silicon/robot/proc/update_upgrades()
...
/datum/hud/silicon/robot/proc/handle_event(event, sender)
...
/mob/living/silicon/robot/updateStatusUi()
