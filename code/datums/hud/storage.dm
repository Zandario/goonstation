...
/datum/hud/storage/clear_master()
	master = null
	..()

/datum/hud/storage/relay_click(id, mob/user, params)
	switch (id)
		if ("boxes")
...
/datum/hud/storage/MouseEntered(atom/movable/screen/hud/H, location, control, params)
	if (!H || H.id != "boxes")
		return
	if (usr)
...
/datum/hud/storage/MouseExited(atom/movable/screen/hud/H)
	if (!H)
		return
	sel.screen_loc = null
...
/*
/datum/hud/storage/mouse_drop(atom/movable/screen/hud/H, atom/over_object, src_location, over_location, over_control, params)
	var/obj/item/I = src.obj_locs[H.screen_loc]
	if (I)
		I.mouse_drop(over_object, src_location, over_location, over_control, params)
*/
/datum/hud/storage/proc/update(mob/user = usr)
	var x = 1
...
/datum/hud/storage/proc/add_item(obj/item/I, mob/user = usr)
	update(user)

/datum/hud/storage/proc/remove_item(obj/item/I)
	remove_object(I)
	update()
