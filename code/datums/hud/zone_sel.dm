...
/datum/hud/zone_sel/New(M, sloc, icon/I)
	..()
	master = M
	if (sloc)
		slocation = sloc
...
/datum/hud/zone_sel/clear_master()
	master = null
	..()

/datum/hud/zone_sel/relay_click(id, mob/user, list/params)
	if (!id || id == "background" || id == "selection")
		return
	src.select_zone(id)

/datum/hud/zone_sel/proc/select_zone(zone)
	if (!zone)
		return
	src.selecting = zone
...
/datum/hud/zone_sel/proc/change_hud_style(icon/new_file)
	if (new_file)
