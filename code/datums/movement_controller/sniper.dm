...
/datum/movement_controller/sniper_look/keys_changed(mob/owner, keys, changed)
...
/datum/movement_controller/sniper_look/process_move(mob/owner, keys)
	if (owner.client)
...
/datum/movement_controller/designator_look/keys_changed(mob/owner, keys, changed)
	if (changed & (KEY_FORWARD|KEY_BACKWARD|KEY_RIGHT|KEY_LEFT|KEY_RUN))
		if (ishuman(owner))
			var/mob/living/carbon/human/H = owner
...
/datum/movement_controller/designator_look/process_move(mob/owner, keys)
	if (owner.client)
		owner.client.pixel_x += input_x * spd
		owner.client.pixel_y += input_y * spd
